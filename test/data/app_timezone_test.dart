...
import 'package:solucast/data/timezone/app_timezone.dart';
...
void main() {
  group('offsetForLocalDate — DST doğruluğu', () {
    ...
    test('Los Angeles: kışın PST (UTC-8), yazın PDT (UTC-7)', () {
      final winter = AppTimeZone.offsetForLocalDate(
          'America/Los_Angeles', DateTime(2026, 1, 15));
      final summer = AppTimeZone.offsetForLocalDate(
          'America/Los_Angeles', DateTime(2026, 7, 15));
      expect(winter, const Duration(hours: -8));
      expect(summer, const Duration(hours: -7));
    });

    test('İstanbul: Türkiye 2016dan beri DST uygulamıyor → yıl boyu +3 sabit', () {
      final winter = AppTimeZone.offsetForLocalDate(
          'Europe/Istanbul', DateTime(2026, 1, 15));
      final summer = AppTimeZone.offsetForLocalDate(
          'Europe/Istanbul', DateTime(2026, 7, 15));
    ...
    test('Oslo (Tromsø tz\'si): AB DST kuralına göre kışın +1, yazın +2', () {
      final winter =
          AppTimeZone.offsetForLocalDate('Europe/Oslo', DateTime(2026, 1, 15));
      final summer =
          AppTimeZone.offsetForLocalDate('Europe/Oslo', DateTime(2026, 7, 15));
      expect(winter, const Duration(hours: 1));
      expect(summer, const Duration(hours: 2));
    });

    test('Sydney: Güney yarıküre — Ocak (yaz) +11 DST, Temmuz (kış) +10 — TERS yönlü', () {
      final januarySummer = AppTimeZone.offsetForLocalDate(
          'Australia/Sydney', DateTime(2026, 1, 15));
      final julyWinter = AppTimeZone.offsetForLocalDate(
          'Australia/Sydney', DateTime(2026, 7, 15));
  ...
  group('DST geçiş günleri — motor çökmeden/anlamsız veri üretmeden çalışır', () {
    test('New York bahar-ileri (2026-03-08, saat 02:00→03:00)', () {
      ...
      final offset = AppTimeZone.offsetForLocalDate(
          'America/New_York', DateTime(2026, 3, 8));
      expect(offset, const Duration(hours: -4));
    });

    test('New York güz-geri (2026-11-01, saat 02:00→01:00)', () {
      final offset = AppTimeZone.offsetForLocalDate(
          'America/New_York', DateTime(2026, 11, 1));
      expect(offset, const Duration(hours: -5));
    });
  });

  group('nowInZone', () {
    test('farklı tz kimlikleri farklı duvar saatleri döndürür (tutarlılık)', () {
