...
void main() {
  ...
  test('replaceActive aktif kaydı değiştirir, liste uzunluğu sabit kalır', () async {
    ...
    expect(state.locations.any((l) => l.name == 'İstanbul'), isFalse,
  ...
  test('replaceActive isim çakışmasında kopyayı düşürür (belirsizlik olmaz)', () async {
    SharedPreferences.setMockInitialValues({});
    final sp = await SharedPreferences.getInstance();
    final c = await makeContainer(sp);
    addTearDown(c.dispose);
    ...
    controller.add(const SavedLocation(
        name: 'Trabzon, Türkiye',
        latitude: 40.9,
        longitude: 39.7,
        timeZoneId: 'Europe/Istanbul'));
    ...
    final state = c.read(locationsProvider);
    final matches =
        state.locations.where((l) => l.name == gps.name).toList();
    expect(matches, hasLength(1), reason: 'aynı isimden tek kayıt kalmalı');
    expect(matches.single.isDeviceLocation, isTrue,
